...
const DataTableCategory = ({ myData, myClass, multiSelectOption, pagination , deleteFunction , editFunction }: any) => {
  ...
  for (const key in myData[0]) {
    ...
    if(key === "category" || key==="subCategory" || key==="image"){
      if(key==="image"){
        columns.push({
          name:   <b>{Capitalize(key.toString())}</b>,
          header: <b>{Capitalize(key.toString())}</b>,
          selector: (row) => <img src={row[key]} alt="" style={{height:"100px" , width:"100px"}} />,
          Cell: editable,
          style: {
  ...
  if (multiSelectOption === true) {
    columns.push({
      ...
      cell: (row) => (
        <div>
          <span>
            <Input type="checkbox" name={row._id} defaultChecked={checkedValues.includes(row._id)} onChange={(e) => deleteFunction( row._id)} />
          </span>
        </div>
      ),
      style: {
