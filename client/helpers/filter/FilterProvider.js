...
const FilterProvider = (props) => {
  ...
  useEffect(() => {
    if(selectedBrands.length!==0){
      router.push(`${window.location.pathname}?category=${selectedCategory}&brand=${selectedBrands}&color=${selectedColor}&size=${selectedSize}&minPrice=${selectedPrice?.min}&maxPrice=${selectedPrice?.max}`, undefined, { shallow: true });
