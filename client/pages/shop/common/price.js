...
const Price = () => {
  ...
  const priceHandle = (value) => {
    if (value) {
      ...
      setValues(value);
      router.push(`${url}?category=${context.state}&brand=${context.selectedBrands}&color=${context.selectedColor}&size=${context.selectedSize}&minPrice=${value[0]}&maxPrice=${value[1]}`, undefined, { shallow: true });
    }
  };
  return (
