...
const updateOrder = async function (req, res) {
  try {
    ...
    try {
      console.log("---------------------------------------------------------");
      console.log("UPDATE ORDER EMAIL DEBUG");
      console.log("Order ID:", updatedOrder.orderId);
      console.log("Req Body:", JSON.stringify(req.body));

      if (!req.body.orderStatus) {
        console.log("No orderStatus in request body.");
      } else {
        const statusLower = req.body.orderStatus.toLowerCase();
        console.log("Status Lower:", statusLower);

        if (statusLower === 'shipped') {
          const awb = req.body.forwardAwb || updatedOrder.forwardAwb || "N/A";
          console.log("Sending Shipped Email with AWB:", awb);

          EmailSendComponent(
            updatedOrder.customerDetails.email,
            "Order Shipped - " + updatedOrder.orderId,
            orderShippedTemplate(updatedOrder, awb)
          );
        } else if (statusLower === 'cancelled') {
          console.log("Sending Cancelled Email");
          EmailSendComponent(
            updatedOrder.customerDetails.email,
            "Order Cancelled - " + updatedOrder.orderId,
            orderCancelledTemplate(updatedOrder)
          );
        } else if (statusLower.includes('return') || statusLower.includes('exchange')) {
          console.log("Sending Return/Exchange Email");
          EmailSendComponent(
            updatedOrder.customerDetails.email,
            "Return Update - " + updatedOrder.orderId,
            returnUpdateTemplate(updatedOrder, req.body.orderStatus)
          );
        } else {
          console.log("Sending Generic Update Email");
          // Fallback for other statuses
          EmailSendComponent(
            updatedOrder.customerDetails.email,
            "Order Status Update - " + updatedOrder.orderId,
            htmlContentForMailTemplate(
              updatedOrder.customerDetails.first_name,
              "Order Status Updated",
              `Your order ${updatedOrder.orderId} status has been updated to: ${req.body.orderStatus}`
            )
          );
        }
      }
